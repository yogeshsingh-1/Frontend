...
const BlogCard = (props: BlogCardProps) => {
  ...
  return (
    <div className="border border-gray-100 border-opacity-50 p-3 w-[70%]  rounded-md shadow-sm">
      ...
      <div className="flex items-center justify-between">
        ...
        <div onClick={() => setIsOpen(true)}>
          {/* <Link to={`/update/${id}`}> */}
          <svg className="w-5 h-5 text-gray-400 dark:text-gray-500 " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <Link to={`/user/blog/${id}`}> <div className="font-semibold  text-lg"><span className=" hover:border-b border-black">{title}</span></div></Link>
