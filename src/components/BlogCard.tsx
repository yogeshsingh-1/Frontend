    ...
    <div className="border border-gray-100 border-opacity-50 p-3 w-[70%]  rounded-md shadow-sm">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div className="ml-2 font-normal text-gray-400 text-sm ">
            {authorName.replace(/\b\w/g, (c) => c.toUpperCase())}
          </div>
        ...
        {isOpen && (
          <div className="fixed inset-0 backdrop-blur-sm z-50 bg-black bg-opacity-40  flex items-center justify-center  ">
            <Update id={id} onClose={() => setIsOpen(false)} />
          </div>
        )}
      </div>
      ...
      <Link to={`/user/blog/${id}`}>
        ...
        <div className="font-semibold  text-lg">
          <span className=" hover:border-b border-black">{title}</span>
        </div>
      </Link>
...
export function Avatar({ name, size = 4 }: { name: string; size?: number }) {
  ...
  return (
    <div
      className={`relative inline-flex items-center justify-center  ${sizeClass} overflow-hidden  rounded-full bg-gray-700`}
    >
